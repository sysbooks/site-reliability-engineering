...
- on-call workと同じくらい、ticketのために引き継ぎをすべき
  - handoff processはチケットハンドラ間の共有状態を維持する
- 割り込みの要因の基本的な振り返りは割り込み率の低下のための良い解決策を与える
- 多くのチームはon-callの引き継ぎとpageのreviewは行うがticketsについてそれらを行うところは少ない
...
- この格言(Respect yourself…)は自動割り込みよりも人間による割り込みの際により強く適用される
- もしticketがうんざりするようなものだった場合は、痛みを緩和するべく以下ポリシーを使用できる
  - あなたのチームがあなたの提供するサービスのサービスレベルを設定する
  - あなたの客に対して努力してもらっても良い
- あなたのチームが顧客からのticketやinterruptsを扱う責任があるなら、あなたの仕事負荷を管理しやすくするためにポリシーを使って良い
- ポリシーの修正は一時的もしくは恒久的に行う
- ポリシーの修正は顧客へのrespectと自分へのrespectのバランスをとるようにすべき
- ポリシーはコードと同様にパワフル
- 例えば、ユーザがほとんど使ってないようなシステムをメンテしている場合、それ関連の割り込みに時間を割くより他の事をしたほうがいいのではないかなど考えてpageを捨てる("pager back" pagerを返す？)などしたほうがよい
  - pageを捨てる以外にもそのcomponentをdeprecateにする、別のものに置き換えるなど、色々な方法がある
- interruptsの主要因を消そうと思えないということは、それはそんなに重要ではないシステムということ
- 時間を食うようなinterruptsがふってきた場合でもあなたの権限無しでできることであれば、リクエストを投げてきた人に投げ返しても良い
- 重要なのは、自分と顧客を両方とも尊重すること
- 要求が"合理的"、"要求を満たすために必要な情報が揃っていること"であるように顧客に指導して、その見返りにこちらのresponseはhelpfulでありtimelyである必要がある
